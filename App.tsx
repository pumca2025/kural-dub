...
const App: React.FC = () => {
  ...
  const handleExport = async () => {
    ...
    // Mark as exported in backend if we have a history ID
    // (handled via historyId state if needed)
  };
  ...
  const parseScript = (text: string): ScriptLine[] => {
    ...
    for (const block of entryBlocks) {
      ...
      lines.push({
        ...
        versions: { spoken: sMatch ? sMatch[1].trim() : '', tanglish: tMatch ? tMatch[1].trim() : '', syncShort: lMatch ? lMatch[1].trim() : '' }
      });
    }
